...
Update `intents.json` with new tags, patterns, and responses. The backend checks the file hash on each API request and retrains automatically when the knowledge base changes. The trained model is saved as `chatbot_model.pkl`.